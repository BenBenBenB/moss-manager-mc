...
class ProjectStatusCommands {
    ...
    private static int listStatuses(CommandContext<ServerCommandSource> context) {
        ...
        var project = MossManApi.getProjectRepository().findAll(0, Integer.MAX_VALUE).stream().filter(p -> p.getTicketPrefix().equalsIgnoreCase(prefix)).findFirst().orElse(null);
    ...
    private static int updateStatus(CommandContext<ServerCommandSource> context) {
        ...
        try {
            ...
            MossManApi.getUpdateProjectStatusesUseCase().execute(project.getId(), rId, newStatuses);
    ...
    private static int removeStatus(CommandContext<ServerCommandSource> context) {
        ...
        try {
            ...
            int migrated = 0;
            for (var ticket : MossManApi.getTicketRepository().findByProjectId(project.getId(), 0, Integer.MAX_VALUE)) {
                if (ticket.getStatus().equalsIgnoreCase(name)) {
                    MossManApi.getTicketRepository().save(new com.mossman.domain.entities.Ticket(
