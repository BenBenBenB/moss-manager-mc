...
public class MailCommand {
    ...
    private static int doListMail(CommandContext<ServerCommandSource> context, int page) {
        ...
        long total = com.mossman.MossManApi.getMailRepository().countByRecipientId(playerId);
        long unread = com.mossman.MossManApi.getMailRepository().countUnread(playerId);
        ...
        List<MailMessage> messages = com.mossman.MossManApi.getMailRepository().findByRecipientId(playerId, offset, pageSize);
    ...
    private static int readMail(CommandContext<ServerCommandSource> context) {
        ...
        try {
            MailMessage msg = com.mossman.MossManApi.getMarkMailReadUseCase().execute(id, playerId);
    ...
    private static int sendMail(CommandContext<ServerCommandSource> context) {
        ...
        try {
            ...
            for (var profile : GameProfileArgumentType.getProfileArgument(context, "player")) {
                ...
                MailMessage saved = com.mossman.MossManApi.getSendMailUseCase().execute(msg);

                // Deliver to online player immediately
                com.mossman.MossManTuiMod.deliverMailNow(saved, source.getServer());
